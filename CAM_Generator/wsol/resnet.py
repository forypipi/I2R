...
class ResNetBCAM(pl.LightningModule):
    ...
    def forward(self, x, labels=None, return_cam=False):
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        feature = x.clone()     # Z, b,c,h,w
        
        att_map_A = self.aggregator_A(feature)
        b, c, h, w = att_map_A.shape
        att_map_A = torch.softmax(att_map_A.view(b, c, h*w), dim=-1)        # A^o, shape:b,M,hw

        att_map_B = self.aggregator_B(feature)
        att_map_B = torch.softmax(att_map_B.view(b, c, h*w), dim=-1)        # A^b,  shape:b,M,hw

        f_pixel = feature.view(b, -1, h*w)  # b,c,hw
        f_image_fore = torch.bmm(att_map_A, f_pixel.permute(0, 2, 1)).mean(dim=1).view(b, -1, 1, 1)     # Z^o
        f_image_back = torch.bmm(att_map_B, f_pixel.permute(0, 2, 1)).mean(dim=1).view(b, -1, 1, 1)     # Z^b
        
        label_fore = self.classifier_A(f_image_fore)                    # fc layer
        label_fore = label_fore.view(label_fore.size(0), -1)            # s^o, shape:b,K

        label_back_rev = self.classifier_A(f_image_back)                # fc layer
        label_back_rev = label_back_rev.view(label_fore.size(0), -1)    # s^b, shape:b,K

        label_back = self.classifier_B(torch.bmm(att_map_B, f_pixel.detach().clone().permute(0, 2, 1)).mean(dim=1).view(b, -1, 1, 1))           # why need detech f_pixel?
        label_back = label_back.view(label_back.size(0), -1)            # b^b, shape:b,K,1
        
        label_fore_rev = self.classifier_B(torch.bmm(att_map_A, f_pixel.detach().clone().permute(0, 2, 1)).mean(dim=1).view(b, -1, 1, 1))
        label_fore_rev = label_fore_rev.view(label_fore.size(0), -1)    # b^o, shape:b,K,1

        

        f_pixel = f_pixel.view(b, -1, h, w)
        att_map_A = torch.sum(att_map_A, dim=1).view(b, 1, h, w)        # sum the heatmap together
        att_map_B = torch.sum(att_map_B, dim=1).view(b, 1, h, w)        # sum the heatmap together

        if return_cam:
            
            feat_map_A = self.classifier_A(feature)         # b,c,h,w to b,K,h,w
            feat_map_B = self.classifier_B(feature)

            normalized_a = normalize_tensor(
                feat_map_A.detach().clone())                # cam map (K classes)
            normalized_b = normalize_tensor(
                feat_map_B.detach().clone())

            cams = normalized_a[range(batch_size), labels]              # return the foreground heatmap related to the label
...
class ResNetCam(pl.LightningModule):
    def __init__(self, block, layers, num_classes=1000,
...
class ResNetSpg(pl.LightningModule):
    def __init__(self, block, layers, num_classes=1000,
                 large_feature_map=False, **kwargs):
...
class ResNetAdl(pl.LightningModule):
    def __init__(self, block, layers, num_classes=1000,
                 large_feature_map=False, **kwargs):
