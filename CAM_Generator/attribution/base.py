...
class Core(torch.nn.Module):
    ...
    def _encode_one_hot(self, targets: torch.Tensor, logits: torch.Tensor):
        '''
        targets: total element number is B
        logits: torch.Tensor in shape [B, 1000], 1000 for ImageNet classes
        '''
...
class VanillaGradient(Core):
    ...
    def get_smoothed_mask(self, img, target_class, samples=25, std=0.15, process=lambda x: x**2):
        ...
        for sample in range(samples):
            noise = torch.empty(img.size(), device=self.device).normal_(0, std)
...
class CAMWrapper(Core):
    ...
    def get_mask(self, img: torch.Tensor,
        ...
        '''
        img: backbone input images, in shape [b,c,h,w]
        target_class: foreground class, in shape [b]
        target_layer: str
        '''
