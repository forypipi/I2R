...
class Decoder(pl.LightningModule):
    ...
    def __init__(self, ppm_scales=[64, 32, 16, 8]):
        ...
        assert self.depth%4==0, 'depth in SETR_MLA should be divided by 4'
        self.unit_depth = self.depth // 4
        ...
        for idx, bin in enumerate(self.pyramid_bins):
            ...
            self.vit_head.append(VIT_MLAHead(
                img_size=bin,
                ln_ch=int((bin/self.patch_size)**2),
                embed_dim=self.embed_dim,
                out_channel=self.reduce_dim,
                mla_channels=256,
            ))
    ...
    def forward(self, feats: List[torch.Tensor], sz: int):
        ...
        for idx, bin in enumerate(self.pyramid_bins):  # [64, 32, 16, 8]
            ...
            merge_feat_bin = [
                merge_feat_bin[self.unit_depth - 1],
                merge_feat_bin[2 * self.unit_depth - 1],
                merge_feat_bin[3 * self.unit_depth - 1],
                merge_feat_bin[4 * self.unit_depth - 1],
                ]
...
class MLAHead(pl.LightningModule):
    def __init__(self, ln_ch, in_channels=1024, mla_channels=256,):
        super(MLAHead, self).__init__()

        self.ln = nn.ModuleList([nn.LayerNorm(normalized_shape=(ln_ch,in_channels), eps=1e-6)] * 4)
        self.mla_1x1 = nn.ModuleList(
            [nn.Sequential(
    ...
    def forward(self, mla_p: List):
        feats = []
        for mla_feat, mla_1x1, ln in zip(mla_p, self.mla_1x1, self.ln):
            mla_feat = self.to_2D(ln(mla_feat))
            mla_feat = mla_1x1(mla_feat)
            feats.append(mla_feat)

        feats[2] = feats[3] + feats[2]
        feats[1] = feats[2] + feats[1]
        feats[0] = feats[1] + feats[0]

        mla_p5 = self.mla_p5(feats[3])
        mla_p4 = self.mla_p4(feats[2])
        mla_p3 = self.mla_p3(feats[1])
        mla_p2 = self.mla_p2(feats[0])

        return torch.cat([mla_p2, mla_p3, mla_p4, mla_p5], dim=1)


class VIT_MLAHead(pl.LightningModule):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    ...
    def __init__(self, 
                 ...
                 **kwargs):
        '''
        combine 4 different layer information from transformer
        '''
        ...
        self.mlahead = MLAHead(ln_ch=ln_ch, in_channels=self.embed_dim, mla_channels=self.mla_channels)
        self.cls = nn.Conv2d(4 * self.mla_channels, out_channel, 3, padding=1)

    def forward(self, inputs):
        x = self.mlahead(inputs)
        x = self.cls(x)
