...
class Decoder(pl.LightningModule):
    ...
    def __init__(self, ppm_scales=[64, 32, 16, 8]):
        ...
        reduce_dim = 256
        embed_dim = 512
        patch_size = 3  # actually patch size = 2^patch_size
        ...
        for idx, bin in enumerate(self.pyramid_bins):
            ...
            self.vit_head.append(ViTPUPHead(
                img_size=bin,
                embed_dim=embed_dim,
...
class ViTPUPHead(pl.LightningModule):
    ...
    def __init__(self, 
                 ...
                 out_channel=2,
                 **kwargs):
        super(ViTPUPHead, self).__init__(**kwargs)
        self.img_size = img_size
        ...
        self.conv_list = nn.ModuleList(
            ...
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),) if i == 0 else 
            nn.Sequential(
        ...
        self.final_conv = nn.Conv2d(256, out_channel, 1, 1)
    ...
    def forward(self, x):
        ...
        for convs in self.conv_list:    # patch_size upsampling
            x = convs(x)
            x = F.interpolate(x, size=x.shape[-1]*2, mode='bilinear', align_corners=True)
